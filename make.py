...
def build():
  #copy the osm-hybrid tree to a build dir
  copy_tree("osm-hybrid", "build")
  ...
  templatefile = open(join('osm-hybrid', 'osm-hybrid.%s.mml' % config["importer"]))
...
def pull():
  #copy the project from mapbox to osm-hybrid
  sanitized_name = re.sub("[^\w]", "", config["name"])
  output_dir = join(config["path"], sanitized_name)
  copy_tree(output_dir, "osm-hybrid", ("layers", ".thumb.png"))

  #load the project file
  project = loads(open(join("osm-hybrid", "project.mml")).read())
  ...
  project_template = open(join("osm-hybrid", "osm-hybrid.%s.mml") % config["importer"], 'w')
  project_template.write(dumps(project, sort_keys=True, indent=2))

  #now delete project.mml
  unlink(join("osm-hybrid", "project.mml"))
